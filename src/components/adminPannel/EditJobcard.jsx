...
const EditJobCardForm = () => {
    ...
    const onSubmitCard = async (e) => {
        ...
        try {
            const response = await axios.put(`${apiBaseUrl}/jobcard/edit-card`, {
        ...
        const fetchJobCardData = async () => {
            try {
                ...
                if (result.status == 200) {
                    ...
                    setCusName(result.data?.data?.customerName || "");
                    setCusAddress(result.data?.data?.customerAddress || "");
                    setCusPhone(result.data?.data?.phoneNumber || "");
                    setMake(result?.data?.data?.Make || "");
                    setPlanType(result?.data?.data?.HP ? "HP" : "KVA")
                    setHp(result?.data?.data?.HP || "");
                    setKva(result?.data?.data?.KVA || "");
                    setRpm(result?.data?.data?.RPM || "");
                    setType(result?.data?.data?.Type || "");
                    setFrame(result?.data?.data?.Frame || "");
                    setSrNo(result?.data?.data?.SrNo || "");
                    setDealerName(result?.data?.data?.DealerName || "");
                    setDealerNumber(result?.data?.data?.DealerNumber || "");
                    setWorks(result?.data?.data?.works || "");
                    setSpares(result?.data?.data?.spares || "");
                    setIndustrialWork(result?.data?.data?.industrialworks || "");
                    setOther(result?.data?.data?.others || "");
                    setAddOns(result?.data?.data?.attachments || []);
                    setWarranty(result?.data?.data?.warranty);
